...
public class AnnotationParser {
	...
	public static List<AnnotationNode> allAnnotations(FieldNode node) {
		ArrayList<AnnotationNode> nodes = new ArrayList<>();
		if (node.visibleAnnotations != null) nodes.addAll(node.visibleAnnotations);
		if (node.invisibleAnnotations != null) nodes.addAll(node.invisibleAnnotations);
		return nodes;
	...
	public static <T> T parseInto(AnnotationNode node, Class<T> targetClass) {
		try {
			...
			if (t instanceof BaseStruct) baseStruct = (BaseStruct) t;
			...
			for (Field declaredField : targetClass.getDeclaredFields()) {
				if (!Modifier.isStatic(declaredField.getModifiers())) {
					...
					if (clazz.equals(String.class)) {
						...
						theUnsafe.putInt(t, offset, (int) o);
						...
						theUnsafe.putChar(t, offset, (char) o);
						...
						theUnsafe.putFloat(t, offset, (float) o);
						...
						theUnsafe.putDouble(t, offset, (double) o);
						...
						Field f = clazz.getDeclaredField(((String[]) o)[1]);
						o = f.get(null);
						theUnsafe.putObject(t, offset, o);
					}
					//
					//
					else {
			...
			theUnsafe.throwException(err); // creates cleaner stacktrace
			throw new RuntimeException(err);
		}
	}
}
